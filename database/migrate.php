 ...
 * 
 * This script runs safe incremental schema updates without dropping or overwriting live production data.
 * Can be run via CLI: php database/migrate.php
 * Or via web by logged-in admin: https://yourdomain/database/migrate.php
 */
...
function outputMsg(string $msg, bool $isCli): void {
    if ($isCli) {
        echo $msg . "\n";
    } else {
        echo "<div style='font-family:monospace;padding:6px;color:#00d2ff;background:#0b2545;margin:2px;'>{$msg}</div>";
    }
}

outputMsg("🔄 Starting safe database migration...", $isCli);

// 1. Ensure all tables exist safely
$tables = [
...
outputMsg("✅ All core tables verified without altering existing data.", $isCli);
outputMsg("🎉 Migration completed successfully!", $isCli);
