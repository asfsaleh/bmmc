...
class MatchingService {
    ...
    public function matchAndNotify(int $requestId, bool $exactMatchOnly = false): array {
        ...
        // 2. Query available donors (Resting period checked: is_available=1 and next_available_date <= CURDATE() or NULL)
        ...
        $sql = "
            ...
            WHERE d.blood_group IN ({$inQuery})
              ...
              AND (d.next_available_date IS NULL OR d.next_available_date <= CURDATE())
              AND d.id NOT IN (
        ...
        $params = array_merge($eligibleGroups, [$requestId, $request['district'], '%' . $request['area'] . '%']);
